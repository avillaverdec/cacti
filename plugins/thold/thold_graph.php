	...
	if (count($result)) {
		foreach ($result as $row) {
			...
			if ($row['thold_enabled'] == 'off') {
				form_alternate_row_color2($thold_bgcolors['grey'], $thold_bgcolors['grey'], $i, 'line' . $row["id"]); $i++;
			}else{
				form_alternate_row_color2($thold_bgcolors[$bgcolor], $thold_bgcolors[$bgcolor], $i, 'line' . $row["id"]); $i++;
			}
...
function thold_show_log() {
	...
	$types = array('High/Low', 'Baseline Deviation', 'Time Based', "Up/Down");
	...
	$total_rows = db_fetch_cell("SELECT
		COUNT(DISTINCT plugin_thold_log.id)
	...
	$sql_query = "SELECT DISTINCT plugin_thold_log.*, host.description AS hdescription, thold_data.name AS name, gtg.title_cache
		FROM plugin_thold_log
		LEFT JOIN host ON plugin_thold_log.host_id=host.id
		LEFT JOIN thold_data ON plugin_thold_log.threshold_id=thold_data.id
		LEFT JOIN graph_templates_graph AS gtg ON plugin_thold_log.graph_id=gtg.local_graph_id
	...
	if (sizeof($logs)) {
		foreach ($logs as $l) {
            if ($l["type"] == 3){
				$name = "Host status";
				$thold_value = $l["threshold_value"];
				$current = $l["current"];
			} else {
				$name = $l["name"];
				$thold_value = $l["threshold_value"] != '' ? thold_format_number($l["threshold_value"]):'N/A';
				$current = $l["current"] != '' ? thold_format_number($l["current"]):'N/A';
			}
			?>
			...
			<td style='white-space:nowrap;'><?php print $name;?></td>
			<td style='white-space:nowrap;'><?php print date("Y-m-d H:i:s", $l["time"]);?></td>
			<td><?php print $thold_value;?></td>
			<td><?php print $current;?></td>
