...
function thold_log($save){
	...
	if (read_config_option('thold_log_cacti') == 'on') {
		...
		if ($save['type'] == 3){
			...
			$enable = db_fetch_row('SELECT monitor FROM host WHERE id = ' . $save['host_id'], FALSE);
		} else {
			...
			$type = $types[$thold['thold_type']];
			$enable = $thold['thold_enabled'];
		}
		...
		case 2:
			...
			break;
		case 3:
			$desc .= ' Description: '.$save["description"];
			$desc .= ' Current: '.$save['current'];
		}
