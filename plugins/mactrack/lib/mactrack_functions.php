...
function build_InterfacesTable(&$device, &$ifIndexes, $getLinkPorts = FALSE, $getAlias = FALSE) {
	...
	$i = 0;
	$sysUpTime = (int) $device["snmp_sysUptime"];
	$frecuency = (int) read_config_option("mt_collection_timing", TRUE) * 60;
	foreach($ifIndexes as $ifIndex) {
		...
		if (!isset($db_interface[$ifIndex]["ifOperStatus"])) {
			if ($ifOperStatus[$ifIndex] == 1) {
				...
				$stateChanges = 0;
				$seconds = $sysUpTime-$ifLastChange[$ifIndex];
				$seconds /= 100;
				$seconds = '- '.round($seconds).' seconds';
				$last_down_time = date('Y-m-d H:i:s', strtotime($seconds));
			}else{
				...
				$seconds /= 100;
				$seconds = '- '.round($seconds).' seconds';
				$last_up_time = date('Y-m-d H:i:s', strtotime($seconds));
			}
		}else{
			...
			if ($db_interface[$ifIndex]["ifOperStatus"] == 0) { /* interface previously not up */
				if ($ifOperStatus[$ifIndex] == 1) {
					...
					if ($db_interface[$ifIndex]["last_down_time"] == '0000-00-00 00:00:00') {
						$last_down_time = '0000-00-00 00:00:00';
					}
				}else{
					...
					if ($db_interface[$ifIndex]["last_up_time"] == '0000-00-00 00:00:00') {
						$last_up_time = '0000-00-00 00:00:00';
					}

					/* if the interface has gone up and down since the last scan */
					if (($sysUpTime > ($frecuency * 100)) && (($sysUpTime-$ifLastChange[$ifIndex]) < ($frecuency * 100))){
						$seconds = $sysUpTime-$ifLastChange[$ifIndex];
						$seconds /= 100;
						$seconds = '- '.round($seconds).' seconds';
						$last_up_time = date('Y-m-d H:i:s', strtotime($seconds));
				...
				if ($ifOperStatus[$ifIndex] == 0) {
					...
					/* if the interface has gone down and up since the last scan */
					if (($sysUpTime > ($frecuency * 100)) && (($sysUpTime-$ifLastChange[$ifIndex]) < ($frecuency * 100))){
						$seconds = $sysUpTime-$ifLastChange[$ifIndex];
						$seconds /= 100;
						$seconds = '- '.round($seconds).' seconds';
...
function xform_dot1q_vlan_associations(&$device, $snmp_readstring = "") {
	...
	if (sizeof($xformArray)) {
		foreach($xformArray as $xformItem) {
			...
			$key = str_replace("mib-2.17.7.1.2.2.1.2.","",$key);
			/* now grab the VLAN */
			$perPos = strpos($key, ".");
...
function perform_mactrack_db_maint() {
	...
	db_execute("DELETE QUICK FROM mac_track_ips WHERE scan_date < '$retention_date'");
...
function order_by_name($stats){
	function cmp($a, $b){
		$bancadaa = substr($a["ifName"], stripos($a["ifName"],".")+1, 1);
		$bancadab = substr($b["ifName"], stripos($b["ifName"],".")+1, 1);
		$numpuertoa = substr($a["ifName"], strrpos($a["ifName"],".")+1);
		$numpuertob = substr($b["ifName"], strrpos($b["ifName"],".")+1);
		if (strpos($a["ifName"],"fe")!==false || strpos($a["ifName"],"ge")!==false){
			if (substr($a["ifName"], 0, 2) == substr($b["ifName"], 0, 2)){
				if ($bancadaa == $bancadab)
					return (int) $numpuertoa > (int) $numpuertob ? 1 : (int) $numpuertoa == (int) $numpuertob ? 0 : -1;
				else 
					return (int) $bancadaa > (int) $bancadab ? 1 : (int) $bancadaa == (int) $bancadab ? 0 : -1;
			} elseif (substr($a["ifName"], 0, stripos($a["ifName"],".")) == "fe" && substr($b["ifName"], 0, stripos($b["ifName"],".")) == "ge"){
				return 1;
			} elseif (substr($b["ifName"], 0, stripos($b["ifName"],".")) == "fe" && substr($a["ifName"], 0, stripos($a["ifName"],".")) == "ge"){
				return -1;
			}
		} else
			return -1;
