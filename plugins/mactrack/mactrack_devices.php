...
function form_mactrack_actions() {
	...
	if (isset($_POST["selected_items"])) {
		...
		} elseif ($_POST["drp_action"] == "8") { /* reload */
			api_mactrack_device_reload($selected_items[0]);
		}
	...
	if ($_POST["drp_action"] == "2") { /* Enable Devices */
		...
		print "	<tr>
				<td class='textArea' bgcolor='#" . $colors["form_alternate1"]. "'>
					<p>Are you sure you want to reload all the info of the following device?</p>
					<p><ul>$device_list</ul></p>
				</td>
			</tr>\n
			";
	...
	if (!isset($device_array)) {
		...
		$save_html = "";
	} elseif ($_POST["drp_action"] == '8' && sizeof($device_array)>1){
		print "<tr><td bgcolor='#" . $colors["form_alternate1"]. "'><span class='textError'>With reload option you must select only one device.</span></td></tr>\n";
		$save_html = "";
	}else{
...
function mactrack_device() {
	...
	if (isset($_GET["scan"])){
		$device_name = db_fetch_cell("SELECT device_name FROM mac_track_devices WHERE device_id=".$_GET['device_id']);
		print $_GET["scan"]==1 ? "<span style='font-size: 12px; font-family: monospace; color:#088A08;'><strong>Port Scanning:</strong> Complete. Rescan have been done on device $device_name.</span><br><br>\n" : "<span style='font-size: 12px; font-family: monospace; color:#ff0000'><strong>Port Scanning:</strong> Fail. Device $device_name couldn't be rescanned.</span><br><br>\n";
	}
...
/* This function will be used to reload the device info when a hardware replacement has been done */
function api_mactrack_device_reload($device_id){
	global $config;
	
	/* Remove all the data of the device except the device itself */
	db_execute("DELETE FROM mac_track_aggregated_ports WHERE device_id=" . $device_id);
	db_execute("DELETE FROM mac_track_interfaces WHERE device_id=" . $device_id);
	db_execute("DELETE FROM mac_track_ips WHERE device_id=" . $device_id);
	db_execute("DELETE FROM mac_track_ports WHERE device_id=" . $device_id);
	...
	/* Scan the new device to load the new data */ 
	$scan = exec("php ".$config["base_path"] . "/plugins/mactrack/mactrack_scanner.php -id=".$device_id);
	
	/* Redirect */
	header("Location: mactrack_devices.php?device_id=" . $device_id . (strpos($scan,"Complete")!==false ? "&scan=1" : "&scan=-1"));
	exit;
}
